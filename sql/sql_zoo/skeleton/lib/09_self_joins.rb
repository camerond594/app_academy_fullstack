...
def lrt_stops
  ...
  execute(<<-SQL)
    SELECT
      stops.id, stops.name
    FROM
      routes JOIN stops ON routes.stop_id = stops.id
...
def craiglockhart_to_sighthill
  ...
  execute(<<-SQL)
    ...
    FROM
      routes a JOIN routes b
        ON (a.company = b.company AND a.num = b.num)
    JOIN(
      routes c JOIN routes d
    ...
    WHERE
      stopa.name = 'Craiglockhart' AND stopd.name = 'Sighthill' AND  stopb.name = stopc.name
  SQL
end
